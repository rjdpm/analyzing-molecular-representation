"""
-----------------------------------------------------------------------------
Train a variational autoencoder for selfies for euclidean space
-----------------------------------------------------------------------------
AUTHOR: Soumitra Samanta                          DATE: Saturday, 13012241130
...
# for train data                           
selfies_vae_model.X_train, _ = _selfies2onehot(
    dict_train_data['selfies'],
    selfies_max_len,
    dict_selfies_tokens,
)
selfies_vae_model.X_train = torch.from_numpy(selfies_vae_model.X_train)
selfies_vae_model.X_train_test = selfies_vae_model.X_train[:num_train_test_samples,:]

# for val data 
selfies_vae_model.X_val, _ = _selfies2onehot(
    dict_val_data['selfies'],
    selfies_max_len,
    dict_selfies_tokens,
)
selfies_vae_model.X_val = torch.from_numpy(selfies_vae_model.X_val)

# for test data 
selfies_vae_model.X_test, _ = _selfies2onehot(
    dict_test_data['selfies'],
    selfies_max_len,
    dict_selfies_tokens,
)
selfies_vae_model.X_test = torch.from_numpy(selfies_vae_model.X_test)

# model train
selfies_vae_model.load_test_network()# load pre-trained best network
selfies_vae_model.count_epoch = selfies_vae_model.epoch_best_network
selfies_vae_model.num_mini_epoch = num_epoch

# train
selfies_vae_model.train()  
