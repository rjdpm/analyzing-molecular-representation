"""
-----------------------------------------------------------------------------
For deep learning architechture and training scripts for hyperspherical space
-----------------------------------------------------------------------------
AUTHOR: Soumitra Samanta                            DATE: Friday, 23012241030
...
class CNN1D_GRU_HSVAE(nn.Module):
    """
    ...
    def __init__(
        ...
        space_transform: bool = False,
        device: str = torch.device("cuda" if torch.cuda.is_available() else "cpu"),
    ) -> None:
        super(CNN1D_GRU_HSVAE, self).__init__()
    ...
    def encoder(
        ...
        sigma_z = F.softplus(self.E_sigma(x)) + 1e-4
        # sigma_z = F.elu(self.E_sigma(x)) + 1.01 ## uncomment this for loading the latent dimension = 20 model
    ...
    def forward(
        ...
        if self.space_transform:
            z = z[:, :-1]/((1 - z[:, -1]) + 1e-20).view(z.size(0), 1)
...
#         self.num_layers_encoder = num_layers_encoder
        ...
        self.result_savepath = ''.join([
            ...
            '_ldisttrb_', self.distribution,
            '_sptrnsfrm_', str(self.space_transform),
    ...
    def train_1epoch(self) ->float:
        ...
        for i in tqdm(range(num_iteration)):# iteration over each minibatch
            ...
            _x_bar, _, _, _, _q_z, _p_z = self.network(_x)# forward
            loss = self.loss_function(_x, _x_bar, _q_z, _p_z)# loss calculation
    ...
    def test(
        ...
        with torch.no_grad():# no need to calculate gradient as its for test only
            for i in tqdm(range(num_iteration)):# iteration over each minibatch
                ...
                _x_bar, _, _, _, _q_z, _p_z = self.network(_x)# forward
                loss = self.loss_function(_x, _x_bar, _q_z, _p_z)# loss calculation
                X_bar[start_idx:start_idx+self.batch_size, :] = _x_bar.to("cpu") # move the result to cpu for gpu memeory management
...
#             idx = np.random.permutation(self.X_train.shape[0])# take random samples from train set to test (As train set generally huge and its will be time consuming to test on whole train data)
            ...
            if((epoch==0) or (epoch_test_val_loss<self.val_loss_best_network)):             
    ...
    def selfies2latent_vector(
        ...
        with torch.no_grad(): 
            for i in tqdm(range(num_iteration)):
                ...
                # reconstruct selfies
                _mu, _sigma = self.network.encoder(_onehot_rep)# forward
                _q_z, _ = self.network.reparametarization(mu=_mu, sigma=_sigma)
                z = _q_z.rsample()
                selfies_recon[start_idx:start_idx+self.batch_size] = z

        return torch.stack(selfies_recon).cpu().numpy()
    ...
    def latent_vector2selfies(
        ...
        with torch.no_grad(): 
            for i in tqdm(range(num_iteration)):
                ...
                _onehot_rep_bar = _onehot_rep_bar.to("cpu").numpy()
                selfies_recon[start_idx:start_idx+self.batch_size] = _onehot2selfies(_onehot_rep_bar, 
                                                                                    dict_selfies_tokens['codomain'],
                                                                                    verbose=False
                                                                                )
        #         # validity checking of reconstructed smiles
