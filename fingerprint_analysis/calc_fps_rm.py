...
__all__ = [
    ...
    'calculate_fingerprint_from_smiles_list',
...
def get_fingerprint(mol, name: str, n_morgan_bits: int = 1024):
    ...
    if name == 'rdkit':
        ...
        return AllChem.GetMorganGenerator(radius=2, fpSize=n_morgan_bits).GetFingerprint(mol)
    elif name == 'atompair':
...
def calculate_fingerprint_from_mol(mol: str, fingerprint_name: str, n_morgan_bits: int = 1024) -> np.ndarray:
    ...
    fp = get_fingerprint(mol, fingerprint_name, n_morgan_bits=n_morgan_bits)
    
    return fingerprint_to_array(fp)

def calculate_fingerprint_from_smiles(smiles: str, fingerprint_name: str, n_morgan_bits: int = 1024) -> np.ndarray:
    
    return calculate_fingerprint_from_mol(Chem.MolFromSmiles(smiles), fingerprint_name, n_morgan_bits=n_morgan_bits)

def calculate_fingerprint_from_smiles_list(smiles_list: List[str], fingerprint_name: str, n_morgan_bits: int = 1024) -> np.ndarray:

    smiles_list = [Chem.MolFromSmiles(smiles) for smiles in smiles_list]
    fps = [calculate_fingerprint_from_mol(mol, fingerprint_name, n_morgan_bits=n_morgan_bits) for mol in smiles_list]
...
def smiles_to_morgan_fps(df: pd.DataFrame, smiles_col: str = "smiles", radii=(0, 1, 2), n_bits: int = 1024) -> pd.DataFrame:
    """
