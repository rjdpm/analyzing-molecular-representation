    ...
    def load_test_network(self, temp_network_path=[]):
        ...
        if os.path.isfile(temp_network_path):
            ...
            print('Loaded pre-trained network checkpoint from "{}"\nepoch: {} train loss: {} val loss: {} test loss: {}' \
                  .format(temp_network_path, self.epoch_best_network, self.train_loss, self.val_loss, self.test_loss) \
                 )
...
#                 _onehot_rep_bar, _, _ = self.network(_onehot_rep)# forward
    ...
    # reconstruc smiles 
    def smiles2latent_vector(self, smiles, tokenize='spacial'):
...
#                 _onehot_rep_bar, _, _ = self.network(_onehot_rep)# forward
                ...
                _q_z, _p_z = self.network.reparametarization(mu=_mu, sigma=_sigma)                
                _z = _q_z.rsample().to("cpu").numpy()
                smiles_recon[start_idx:start_idx+self.batch_size] = _z
                

        return np.stack(smiles_recon)
    #------------------------------------------------------

    # reconstruc smiles 
    def latent_vector2smiles(self, x_latent):
