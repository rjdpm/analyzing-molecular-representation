...
class PARSE_SMILES():
    ...
    def load_smiles_from_parquet(smiles_data_filename, smiles_column_ids='smiles', max_string_len=150, smiles_check_flag=0, cannonical_check=0, n_data = 'all', verbose=0):
        ...
        df = pd.read_parquet(smiles_data_filename)
    ...
    @staticmethod
    def smiles2onehot_vector(smiles, char_indices, max_string_len= 150, padding='right', tokenize='spacial', verbose=0):
...
#             check_ids = [Chem.MolFromSmiles(smile) is not None for smile in smiles]
    ...
    @staticmethod
    def draw_molecules(smiles, plot_nrows_module=10, plot_subimg_size=(500, 500), legends=[]):
        ...
        # drow molecules
        if len(legends):
            img = Draw.MolsToGridImage(mol, molsPerRow=plot_nrows_module, subImgSize=plot_subimg_size, legends=legends, returnPNG=False, maxMols = 500)
        else:
            img = Draw.MolsToGridImage(mol, molsPerRow=plot_nrows_module, subImgSize=plot_subimg_size, returnPNG=False, maxMols = 500)
