...
# for train data                           
smiles_vae_model.X_train = torch.from_numpy(listsmi2onehot(smiles_list=smiles_train,
                                                           char_list=smiles_char,
                                                           max_str_len=max_smiles_length
                                                           ))

smiles_vae_model.X_train_test = smiles_vae_model.X_train[:num_train_test_samples,:]
# for val data 
smiles_vae_model.X_val = torch.from_numpy(listsmi2onehot(smiles_list=smiles_val,
                                                         char_list=smiles_char,
                                                         max_str_len=max_smiles_length
                                                         )
                                          )
# for test data 
smiles_vae_model.X_test = torch.from_numpy(listsmi2onehot(smiles_list=smiles_test,
                                                          char_list=smiles_char,
                                                          max_str_len=max_smiles_length
                                                          )
                                           )
...
# model train
smiles_vae_model.count_epoch = 0
smiles_vae_model.num_mini_epoch = num_epoch

smiles_vae_model.load_test_network()# load pre-trained best network

#Saving the Network File
source_file = os.path.abspath(__file__)
# destination_file = ''.join([smiles_vae_model.result_savepath, '/train_script_', smiles_vae_model.result_save_filename,'.py'])
# shutil.copy(source_file, destination_file)
with open(source_file, 'rb') as fp:
    train_script = fp.read()
    smiles_vae_model.train_script = pickle.dumps(train_script)

# train
smiles_vae_model.train()                   