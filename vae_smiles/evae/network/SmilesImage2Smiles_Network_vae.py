...
from data_utils_smiles import listsmi2onehot, float_onehot_encodings2smiles
...
class SmIm2Sm_Network():
    ...
    #         network = nn.DataParallel(network)
        ...
        self.best_network_save_filename = ''.join(['best_', self.result_savepath.split('/')[-2]]).replace('.','_')
...
#             idx = np.random.permutation(self.X_train.shape[0])# take random samples from train set to test (As train set generally huge and its will be time consuming to test on whole train data)
            ...
            if((self.count_epoch==0) or (self.val_loss_best_network>epoch_test_val_loss)):             
                ...
                self.epoch_best_network = self.count_epoch+1
    ...
    def save_network(self, epoch=0,
        ...
        torch.save({\
                    'epoch': epoch,
                    'best_epoch': self.epoch_best_network,
    ...
    def load_test_network(self, temp_network_path=[]):
        ...
        if os.path.isfile(temp_network_path):
            ...
            checkpoint = torch.load(temp_network_path, map_location=self.device, weights_only=False)
            #--------------------------------------------------
            ...
            if 'best_epoch' in checkpoint.keys():
                self.epoch_best_network = checkpoint['best_epoch']
            else:
            ...
            print('Loaded pre-trained network checkpoint from "{}"\nepoch: {} train loss: {} val loss: {} test loss: {}' \
                  .format(temp_network_path, self.count_epoch, self.train_loss[self.count_epoch-1], self.val_loss[self.count_epoch-1], self.test_loss[self.count_epoch-1]) \
                 )
    ...
    def smiles_reconstruct_molecule(self, smiles, tokenize='', draw_num_samples=100, plot_nrows_module=10, save_path='temp_results/', dataset_name='temp'):
        ...
        with torch.no_grad(): 
            for i in tqdm(range(num_iteration)):
                ...
                _onehot_rep = listsmi2onehot(_smiles, char_list=self.smiles_char_map['domain'], max_str_len=self.max_string_len)
...
#                 _onehot_rep_bar, _, _ = self.network(_onehot_rep)# forward
                ...
                smiles_recon[start_idx:start_idx+self.batch_size] = float_onehot_encodings2smiles(_onehot_rep_bar, self.smiles_char_map['co-domain'])
    ...
    #------------------------------------------------------
     ...
     # projection function (latent space representation)
    def smiles2latentspace_representation(self, smiles, verbose=False):
        ...
        num_iteration = int(np.ceil(float(num_test_samples)/self.batch_size)) 
        if verbose:
            iterator = tqdm(range(num_iteration))
        else:
            iterator = range(num_iteration)

        # to store the latent space representation
        X_proj = torch.zeros(num_test_samples, self.network.dims_latent)
        X_mean = torch.zeros(num_test_samples, self.network.dims_latent)
        X_sigma = torch.zeros(num_test_samples, self.network.dims_latent)

        print('<===smiles to latent space representation===>')
        with torch.no_grad():
            for i in iterator:
                start_idx = (i*self.batch_size)%num_test_samples
                _smiles = smiles[start_idx:start_idx+self.batch_size]# take minibatch
                ...
                mu, sigma = self.network.encoder(_onehot_rep)
                X_proj[start_idx:start_idx+self.batch_size, :] = self.network.reparametarization(mu, sigma).to("cpu")
                X_mean[start_idx:start_idx+self.batch_size, :] = mu.to("cpu")
                X_sigma[start_idx:start_idx+self.batch_size, :] = sigma.to("cpu")

        return X_proj.numpy(), X_mean.numpy(), X_sigma.numpy()
    #------------------------------------------------------

    # reconstruc smiles 
    def latent_space2smiles(self, x_latent):

        self.network.eval()# switch to test mode

        # minibatch settings (don't need data shuffle)
        num_test_samples = len(x_latent)
        num_iteration = int(np.ceil(float(num_test_samples)/self.batch_size))
        ...
        with torch.no_grad(): 
            for i in tqdm(range(num_iteration)):
                start_idx = (i*self.batch_size)%num_test_samples
                _x_latent = x_latent[start_idx:start_idx+self.batch_size]# take minibatch
                _x_latent = torch.from_numpy(_x_latent).type(torch.float32).to(self.device)# data type conversion and move data to gpu (if available)

                # reconstruct smiles
                _onehot_rep_bar = self.network.decoder(_x_latent)# forward                
                _onehot_rep_bar = _onehot_rep_bar.to("cpu").numpy()
